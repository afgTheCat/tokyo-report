...
\end{gather}
which is the same as the inverse of the Fourier transformation multiplied by \(n\). Thus by writing \(e^{2{\pi}i/N}\) in line 9 instead of \(e^{-2{\pi}i/N}\) and by multiplying each term of the end result by \(1/n\) given that the initial input had size \(n\), we obtain the original \(x\), within the same time complexity.
\subsection*{Importance and Usage}
The FFT is widely used among different fields of engineering, computer science and mathematics. In signal processing it is used at transforming the input sequence into the frequency domain.
...
This model further generalizes the previous one by lifting the restriction on the number of non-placebo cookies, still requiring every cookie to induce a drift in the same \(\ell \in \mathbb{Z}^d\setminus\{0\}\) direction. That is
\begin{gather}
    \sum_{e\in\mathcal{M}}\omega(0, e, i)e \cdot \ell \geq 0 \quad \mathbb{P}\text{-a.s. for all } i \in \mathbb{N}.
\end{gather}
Cookies satisfying the above inequality are called  \(\ell\)-positive cookies. If we assume that condition \ref{SE} holds, the one dimensional version of the model has also shown to obey the strong law of large numbers, as shown in \cite{KZ13}.

\subsection*{Boundedly many positive or negative cookies per site}
This is probably the most widely studied model of ERWs. It was introduced in \cite{BS08}. In this model there is fixed bound after which there are only placebo cookies left in the each cookie stack. Under conditions \ref{IID} and \ref{WEL} in one dimension, using a connection with branching processes, it has been shown that the process obeys the strong law of large numbers. The exact circumstances under which the model is ballistic have also been identified. Limiting theorems in connection to Brownian motion have also been devised. To verify these assessments, we refer to \cite{KZ13} \cite{BS08}, \cite{KZ08} and \cite{MTPV}.

\newpage

\section*{Research plan}
My main field of interest is probability theory. I have also spent considerable time studying the reinforcement learning. As these two fields of mathematics are not closely related, I will write about them separately.
\subsection*{Probability theory}
My main research interest in probability theory are the stochastic processes. I find non-Markovian processes such as excited random walks or reinforced random walks particularly intriguing. As neuron firings can be modeled with reinforced random walks my aim is to use my knowledge of stochastic processes in the pursuit of understanding the human brain.

In \textbf{Theme C} I was tasked to describe how mathematical informatics can help preparing for infectious diseases. Tough previously I did not know anything about the subject, I really enjoyed learning about it. The sources I looked into mostly used differential equations when modeling the spread of viruses/bacterias. I am sure that stochastic models can be just as useful when modeling such events as using more traditional approaches and I want to continue learning about these techniques. I even created a short simulation based on competing exponential variables, their memorylessness and the distribution of their minimum value to turn the problem into a five dimensional stochastic progress, but due to limitation on number of pages submittable I did not include it in my answer.

Another branch of probability theory I am planning to study is random graphs theory. Random graphs can model interconnected systems such as the internet, forest fires, societies etc., and understanding them allows us to tackle related problems such as internet routing, fire suppression or prevention the spread of deadly diseases. In my simulation I used the Watts–Strogatz model.
\subsection*{Reinforcement learning}
Reinforcement learning is in large part inspired by psychological literature on Pavlovian and instrumental conditioning. The agent in the reinforcement learning framework performs actions with the goal of maximizing the \textit{return}.  Increasingly, techniques of reinforcement learning are used in neuroscience to model the brain. I want to study the interdisciplinary dynamics between these two fields. I hope that reinforcement learning combined with my previously stated aim of using stochastic processes can help understanding the brain both on a micro- and on a macro level.

I also want to explore new areas where reinforcement learning can be applied. One such possible area is the efficient emergency supply distribution during disasters. 

\newpage

\section*{Answer to Theme C}
Mathematical informatics can help immensely to prepare for and to handle infectious diseases. Though the tools of bioinformatics are useful to investigate properties of the pathogens and to develop vaccines against it, in this section we choose to focus on other methods to combat epidemics. Epidemic containment efforts have four phases: \textbf{preparedness}, \textbf{outbreak investigation}, \textbf{response} and \textbf{evaluation}. These phases contain a wide range of tasks, such as inventory management for essential medical supply, surveillance system design, the scheduling of vehicles used to transport medical supply and identification of potential bottlenecks.
...
\subsection*{Other tasks and acknowledgement}
...
Writing this section was largely inspired by \cite{LMCJ}. The above figures can be seen as an independent verification of their computational results.
...
\begin{thebibliography}{1}
    ...
    \bibitem{KZ08}
    E.~Kosygina and M.~P.~W. Zerner
    \newblock Positively and negatively excited random walks on integers, with branching processes.
    \newblock In {\em Electronic Journal of Probability, vol 13, no. 64}, pages. 1952--1979. 2008

    \bibitem{MTPV}
    Mountford, Thomas and Pimentel, Leandro and Valle, Glauco
    \newblock On the speed of the one-dimensional excited random walk in the transient regime
    \newblock In {\em Latin American Journal of Probability and Mathematical Statistics, vol 2}. 2006


    \bibitem{LMCJ}
    Liu, Ming and Cao, Jie and Liang, Jing and Chen, MingJun
    \newblock Epidemic-logistics Modeling: A New Perspective on Operations Research. 2020
